...
static int elementalx_powersave_bias_setspeed(struct cpufreq_policy *policy,
...
static void elementalx_powersave_bias_init_cpu(int cpu)
{
...
static void elementalx_powersave_bias_init(void)
{
	int i;
	for_each_online_cpu(i) {
		elementalx_powersave_bias_init_cpu(i);
	}
}
...
skip_this_cpu:
		...
		elementalx_powersave_bias_init();
	} else {
		for_each_online_cpu(cpu) {
			if (lock_policy_rwsem_write(cpu) < 0)
				continue;
			...
			if (dbs_info->cur_policy) {
				...
				elementalx_powersave_bias_setspeed(
					dbs_info->cur_policy,
					NULL,
					input);
				   ...
				   unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		if (!elementalx_powersave_bias_setspeed(
